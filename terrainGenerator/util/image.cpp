...
bool CIMAGE::Create( unsigned int uiWidth, unsigned int uiHeight, unsigned int uiBPP )
{
	...
	if( !m_ucpData )
	{
		cout << "Out of memory for image memory allocation" << endl;
		return false;
	}
...
bool CIMAGE::LoadData( char* szFilename )
{
	...
	if( fread( m_ucpData, sizeof( unsigned char ), iSize, pFile ) != ( unsigned )iSize )
	{
		...
		cout << " corrupted, could not read all data\n" << endl ;
		if( m_ucpData )
		{
			delete[] m_ucpData;
			m_ucpData = NULL;
	...
	else if( memcmp( m_ucpData, g_ucUTGAcompare, 12 )==0 )
	{
		...
		if( !LoadUncompressedTGA( ) )
		{
			...
			cout << "Could not load uncompressed TGA \n";
			if( m_ucpData )
			{
				delete[] m_ucpData;
				m_ucpData= NULL;
	...
	else if( memcmp( m_ucpData, g_ucCTGAcompare, 12 )==0 )
	{
		...
		if( !LoadCompressedTGA( ) )
		{
			...
			cout << "Could not load compressed TGA \n";
			return false;
		}
	}
	...
	{
		cout << " not a supported image type\n";
		if( m_ucpData )
		{
			delete[] m_ucpData;
			m_ucpData= NULL;
...
bool CIMAGE::Save( char* szFilename )
{
	...
	cout << "image has been successfully saved"<< endl;
	return true;
}
...
bool CIMAGE::SaveBMP( char* szFilename )
{
	...
	if( !pFile )
	{
		cout << "Could not open a file to save image in"<< endl;
		return false;
	}
	...
	bitmapInfoHeader.uiCompression = 0x0000;
